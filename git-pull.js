...
/**
 * Pulls the Bitburner runtime files directly from this repository.
 *
 * @param {NS} ns
 */
export async function main(ns) {
    ...
    if (ns.getHostname() !== "home") {
        ns.tprint("ERROR: git-pull.js must be run on home.");
        return;
    }
    ...
    if (!options["no-self"]) {
        targets.push({ remote: "git-pull.js", local: selfFile });
    }
...
function getValidationError(remoteFile, contents) {
    ...
    if (
        remoteFile === "git-pull.js" &&
        (!contents.includes(GITHUB_OWNER) ||
            !contents.includes(GITHUB_REPOSITORY))
    ) {
        return "git-pull.js does not identify the expected repository";
    }
...
function printHelp(ns) {
    ns.tprint("Bitburner-Automation GitHub updater");
    ns.tprint("Usage: run git-pull.js [--branch main] [--no-self]");
    ns.tprint("");
    ns.tprint("Downloads:");
