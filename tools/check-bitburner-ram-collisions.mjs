...
const apiNames = new Set(
    [...dts.matchAll(/^\s*(?:readonly\s+)?([A-Za-z_$][\w$]*)\s*\(/gm)].map((m) => m[1]),
);
const localFunctions = new Set(
    [...source.matchAll(/\b(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*\(/g)].map((m) => m[1]),
);
...
console.log(
    `RAM collision check OK (${localFunctions.size} local functions vs ${apiNames.size} API method names)`,
);
